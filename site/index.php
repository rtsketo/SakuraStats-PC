...
<body>
        ...
        <div class="container">
            <div class="row">
                ...
                <div class="col-xs-2 switch text-right align-middle">
                    <button class="btn-sm btn-danger" onclick="window.location.href='/actio.php'" style="color:beige"> Switch Stats </button>
                </div>
            </div>
        </div>
    </div>
    ...
    <table class="container table-condensed" id="QQmas">
        <thead>
            <tr>
                <th>
                    <h1>Member</h1></th>
                <th>
                    <h1><center>Rank</center></h1></th>
                <th>
                    <h1><center>WWR</center></h1></th>
                <th>
                    <h1><center>Donations</center></h1></th>
                <th>
                    <h1><center>Trophies</center></h1></th>
                <th>
        ...
        <tbody class="switcho">
            ...
            $results = $db->query('SELECT name,CAST(wins AS float) / CAST(wars AS float) AS "ratio", dona,rece,troph,contr,role, vip FROM player WHERE isin = 1 ORDER BY vip DESC');
            while ($row = $results->fetchArray(SQLITE3_ASSOC)) {
                ...
                if($row['role']>.75) echo '<small><ooy>Leader</ooy>';
                elseif($row['role']>.55) echo '<small><hoy>Elder&nbsp</hoy>';
                else echo '<small>Member';
                echo '</small></center></td>';
                
                echo '<td><center>'.round($row['ratio']*100).'<ooy>%</ooy></center></td>';
